...
- `FeedAddActivityRequest.expiresAt` takes a `DateTime?` rather than a `String?`, and the SDK sends it as UTC RFC3339. The API parses this field as strict RFC3339, so a string built from a local `DateTime` — which carries no timezone — was rejected outright; pass the `DateTime` and the conversion is handled for you
