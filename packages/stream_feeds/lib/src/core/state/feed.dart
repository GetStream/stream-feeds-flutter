...
class Feed with Disposable {
  ...
  Future<Result<FeedData>> getOrCreate() async {
    ...
    return result.when(
      success: (feedData) {
        ...
        return Result.success(feedData.feed);
      },
  ...
  Future<Result<FeedData>> updateFeed({
    ...
    return result.when(
      success: (feedData) {
        _stateNotifier.onFeedUpdated(feedData);
  ...
  //   required api.FeedAddActivityRequest request,
  //   void Function(dynamic payload, double progress)? attachmentUploadProgress,
  // }) async {
  //   final result = await activitiesRepository.addActivity(
  //     feedId: query.fid,
  //     request: request,
  //   );
  //
  ...
  //       _stateNotifier.onActivityAdded(activityData);
  ...
  Future<Result<ActivityData>> updateActivity({
    ...
    return result.when(
      success: (activityData) {
        _stateNotifier.onActivityUpdated(activityData);
        return Result.success(activityData);
      },
      failure: Result.failure,
    );
  ...
  Future<Result<void>> deleteActivity({
    ...
    return result.when(
      success: (_) {
        _stateNotifier.onActivityDeleted(activityId);
        return const Result.success(null);
      },
      failure: Result.failure,
    );
  ...
  Future<Result<BookmarkData>> addBookmark({
    ...
    return result.when(
      success: (bookmarkData) {
        _stateNotifier.onBookmarkAdded(bookmarkData);
  ...
  Future<Result<BookmarkData>> deleteBookmark({
    ...
    return result.when(
      success: (bookmarkData) {
        _stateNotifier.onBookmarkRemoved(bookmarkData);
        return Result.success(bookmarkData);
      },
      failure: Result.failure,
    );
  ...
  Future<Result<ModelUpdates<FeedMemberData>>> updateFeedMembers({
    ...
    return result.when(
      success: (modelUpdates) {
        _memberList.notifier.onMembersUpdated(modelUpdates);
        return Result.success(modelUpdates);
      },
      failure: Result.failure,
    );
  }
  ...
  Future<Result<ActivityData>> repost({
    ...
    return result.when(
      success: (repostedActivity) {
        _stateNotifier.onActivityAdded(repostedActivity);
        return Result.success(repostedActivity);
      },
      failure: Result.failure,
    );
  }
  ...
  Future<Result<List<ActivityData>>> queryMoreActivities({int? limit}) async {
    ...
    return result.when(
      success: (feedData) {
        ...
        return Result.success(feedData.activities.items);
      },
      failure: Result.failure,
    );
  }
  ...
  Future<Result<FollowData>> follow({
    ...
    api.SingleFollowRequestPushPreference? pushPreference,
  }) async {
    final request = api.SingleFollowRequest(
    ...
    return result.when(
      success: (followData) {
        ...
        return Result.success(followData);
      },
      failure: Result.failure,
    );
  }
  ...
  Future<Result<void>> unfollow({required FeedId targetFid}) async {
    ...
    return result.when(
      success: (_) {
        _stateNotifier.onUnfollow(sourceFid: query.fid, targetFid: targetFid);
        return const Result.success(null);
      },
      failure: Result.failure,
    );
  ...
  Future<Result<FeedsReactionData>> addCommentReaction({
    ...
    return result.when(
      ...
      success: (tuple) => Result.success(tuple.$1),
      failure: Result.failure,
    );
  }
  ...
  Future<Result<ActivityData>> createPoll({
    ...
    final pollResult = await pollsRepository.createPoll(request);

    return pollResult.when(
      success: (poll) async {
        final request = api.AddActivityRequest(
          fids: ['${query.fid.group}:${query.fid.id}'],
          pollId: poll.id,
          type: activityType,
        );
