        ...
        return PollOptionItem(
          ...
          onChanged: (checked) {
            ...
            switch (poll.votingMode) {
              case PollVotingMode.disabled:
                break;
              // Do nothing
              case PollVotingMode.all:
                return _handleVoteAction(option, checked: checked);
              // Note: We don't need to remove the other votes in the unique
              // voting mode as the backend handles it.
              case PollVotingMode.unique:
                return _handleVoteAction(option, checked: checked);
  ...
  Widget build(BuildContext context) {
    ...
    final control = ExcludeFocus(
      child: Checkbox(
        ...
        checkColor: colorTheme.accentPrimary,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
        side: BorderSide(color: colorTheme.accentPrimary),
                ...
                children: <Widget>[
                  Row(
                    ...
                    children: <Widget>[
                      ...
                      if (poll.votingVisibility == 'public')
                        OptionVoters(
                  ...
                  if (showProgressBar)
                    OptionVotesProgressBar(
                      ...
                      trackColor: colorTheme.barsBg,
                      valueColor: switch (poll.isOptionWinner(option)) {
                        true => colorTheme.accentInfo,
                        false => colorTheme.accentPrimary,
                      },
                    ),
                ],
              ),
            ...
            (user) {
              ...
              return Positioned(
                ...
                child: Container(
                  ...
                  child: UserAvatar.small(user: user),
                ),
              );
            },
          ),
...
extension on PollData {
  ...
  bool hasCurrentUserVotedFor(PollOptionData option) {
    return ownVotesAndAnswers.any((it) => it.optionId == option.id);
  }

  int voteCountFor(PollOptionData option) {
    return voteCountsByOption[option.id] ?? 0;
  }

  double? voteRatioFor(PollOptionData option) {
    if (voteCount == 0) return null;
    return (voteCountsByOption[option.id] ?? 0) / voteCount;
  }

  bool isOptionWinner(PollOptionData option) {
    if (voteCountsByOption.isEmpty) return false;
